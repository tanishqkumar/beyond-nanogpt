...
def loss_fn(policy_net, value_net, 
    ...
    batch_values = batch_values.detach().squeeze(-1)
    batch_next_values = batch_next_values.detach().squeeze(-1)
    ...
    entropy = -(new_probs * new_logprobs_all).sum(dim=-1) # [b, T]
...
def train(nsteps=100, batch_size=64, max_rollout_len=50, lr=1e-3, gamma=0.99, hidden_dim=128, early_stop=False, wandb_log=False, verbose=False, NSTATES=4, ngrads_per_step=3, plot=False):
    ...
    for step in tqdm(range(nsteps)):
        ...
        batch_states = []
        batch_next_states = []
        batch_dones = []  # both will be [b, sm]
        true_lens = torch.zeros(b, device=device)  # entry i is true len of batch element i, use this with arange to mask mask of size [b, s]
        
        if verbose and step % 5 == 0:
            print(f"\nGenerating batch {step} of rollouts...")
            
        for batch_idx in range(b):
            with torch.no_grad():  # in contrast to dqn which stores a buffer where we may be learning from 
                i = 0 
                
                rollout_rewards = torch.zeros(max_rollout_len).to(device)
                rollout_logprobs = torch.zeros(max_rollout_len).to(device)
                rollout_actions = torch.zeros(max_rollout_len, dtype=torch.long).to(device)
        ...
        for _ in range(ngrads_per_step): 
