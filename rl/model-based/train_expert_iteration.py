...
# buffer stores/feeds batches for policy, ie. (s, a) pairs so [sdim, adim]
class Buffer: 
...
def MCTS(
    env: gym.Env, 
    state: torch.Tensor, 
    policy_net: nn.Module, # gives priors for exploration
    value_net: nn.Module, # used to compute values of nodes that we write in 
    num_sims: int = 100, 
...
def train(
    ...
    for step in range(cfg.nsteps): 
        for _ in range(cfg.num_rollouts): 
            ...
            s, _ = env.reset() 
            while not done: 
                a_next = MCTS(env.clone(), s, value_net)
                buffer.push((s, a_next))
                _, _, done, _, _ = env.step(a_next)

        for _ in range(cfg.num_policy_updates_per_step): 
            state_batch, opt_action_batch = buffer.get_batch(cfg.bsz)
            loss = F.CrossEntropLoss(policy_net(state_batch), opt_action_batch)
            loss.backward()
            policy_opt.step()
